...
/// Mục đích        :Hiển thị danh sách phòng ban
/// Ngày tại        :08/04/2020
...
{
    /// <summary>
    /// Xử lý dữ liệu danh sách phòng ban
    /// </summary>
    ...
    public partial class DsPhongBan : DotNetNuke.Entities.Modules.UserModuleBase
    {
        ...
        /// <summary>
        /// Cập nhật thông tin chức vụ (chuyển đến trang Edit)
        /// </summary>
        ...
        protected void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                //Lấy danh sách ID chức vụ trên danh sách
                ...
                //Xóa danh sách chức vụ đã chọn
                int vCountPhongHopDaXoa = 0;
                string oErrorMessage = "";
                foreach (int vChucVuId in vChucVuIds)
                {
                    if (!vPhongBanControllerInfo.KiemTraphongBanDangDuocSuDung(vChucVuId, out oErrorMessage))
                    {
                        vPhongBanControllerInfo.XoaPhongBan(vChucVuId, out oErrorMessage);
                        vCountPhongHopDaXoa += 1;
                    }
                }
                ClassCommon.ShowToastr(Page, "Xóa thành công " + vCountPhongHopDaXoa + " phòng ban!", "Thông báo", "success");
        ...
        {
            HtmlAnchor html = (HtmlAnchor)sender;
            int vPhongBanId = int.Parse(html.HRef);
            string oErrorMessage = "";
            UserInfo _User = new UserInfo();
            try
            {
                if (vPhongBanControllerInfo.KiemTraphongBanDangDuocSuDung(vPhongBanId, out oErrorMessage))
                {
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa không thành công, phòng ban đã được sử dụng!", "Thông báo", "error");
                }
                else
                {
                    vPhongBanControllerInfo.XoaPhongBan(vPhongBanId, out oErrorMessage);
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa phòng ban thành công!", "Thông báo", "success");
                    LoadDanhSach(vCurentPage);
                }
            }
            catch (Exception ex)
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            //Khoi tao dictionary
            if (vSearchContent != "" ||  vSearch_DONVI_ID != -1)
            {
        ...
        protected void LoadDanhSach(int pCurentPage)
        {
            ...
            {                
                ...
                if (Session[vMacAddress + PortalSettings.ActiveTab.TabID + _currentUser.UserID + "_SearchContent"] != null)
                {
                    ...
                    {
                        vDonViId = int.Parse(vDictSearch["DONVI_ID"]);
                    }
                }

                List<PhongBan> vPhongBanInfos = vPhongBanControllerInfo.GetDanhSachPhongBan(vContentSearch, vDonViId);

                if (ViewState["keysort_phongban"] != null && ViewState["typesort_phongban"] != null)
                {
                    string key = ViewState["keysort_phongban"].ToString();
                    string type = ViewState["typesort_phongban"].ToString();

                    if (key == "TenPhongBan" && type == "ASC")
                        vPhongBanInfos = vPhongBanInfos.OrderBy(x => x.TENPHONGBAN).ToList();
                    if (key == "TenPhongBan" && type == "DESC")
                        vPhongBanInfos = vPhongBanInfos.OrderByDescending(x => x.TENPHONGBAN).ToList();                                      
                }

                if (vPhongBanInfos != null)
                {
                    dgDanhSach.VirtualItemCount = vPhongBanInfos.Count;
                    dgDanhSach.DataSource = vPhongBanInfos;
        ...
        protected void dgDanhSach_SortCommand(object source, DataGridSortCommandEventArgs e)
        {
            ...
            ViewState["keysort_phongban"] = e.SortExpression;
            ViewState["typesort_phongban"] = sortDirection;
            LoadDanhSach(0);
        }
