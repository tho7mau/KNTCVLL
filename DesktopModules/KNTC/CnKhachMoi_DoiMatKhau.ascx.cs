...
/// Mục đích        :Đổi mật khẩu khách mời
/// Ngày tại        :03/04/2020
...
{
    public partial class CnKhachMoi_DoiMatKhau : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Properties
        int vKhachMoiId;
        ...
        KhachMoiController vKhachMoiController = new KhachMoiController();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var vKhachMoiInfo = vKhachMoiController.GetKhachMoiTheoId(vKhachMoiId);
                if (vKhachMoiInfo != null)
                {
                    this.ModuleConfiguration.ModuleControl.ControlTitle = "<a href='" + Globals.NavigateURL() + "' class='title-link'>Quản lý khách mời</a> / Đổi mật khẩu / " + vKhachMoiInfo.TENNGUOIDUNG;
                }
            }
            catch (Exception ex)
            {
        ...
        public void SetFormInfo(int pNguoiDungId)
        {
            try
            {
                ...
                var vKhachMoiInfo = vDataContext.NGUOIDUNGs.Where(x => x.NGUOIDUNG_ID == pNguoiDungId).FirstOrDefault();
                if (vKhachMoiInfo != null)
                {
                    textTenDangNhap.Text = vKhachMoiInfo.Username;
                    textHoTenNguoiDung.Text = vKhachMoiInfo.TENNGUOIDUNG;
                }            
            }
            catch (Exception Ex)
            {
        ...
        public void CapNhat(int pNguoiDungID)
        {
            try
            {
                ...
                if (vKhachMoiInfo != null)
                {
                    UserInfo vUserInfo = UserController.GetUserById(this.PortalId, vKhachMoiInfo.UserId ?? -1);
                    ...
                    {
                        Session[vMacAddress + TabId.ToString() + "_Message"] = "Đổi mật khẩu khách mời thành công";
                        Session[vMacAddress + TabId.ToString() + "_Type"] = "success";
                        Response.Redirect(Globals.NavigateURL());
                    }
                    else
                    {
                        Session[vMacAddress + TabId.ToString() + "_Message"] = "Đổi mật khẩu khách mời không thành công";
