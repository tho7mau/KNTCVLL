...
{
    /// <summary>
    /// Xử lý dữ liệu danh sách chức vụ aa
    /// </summary>
    ...
    public partial class DsChucVu : DotNetNuke.Entities.Modules.UserModuleBase
    {
        ...
        ChucVuController vChucVuController = new ChucVuController();
        ...
        protected void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    vChucVuController.XoaChucVu(vId, out oErrorMessage);
                    vCountDaXoa += 1;
                }
                ClassCommon.ShowToastr(Page, "Xóa thành công " + vCountDaXoa + " chức vụ!", "Thông báo", "success");
                LoadDanhSach(1, vPageSize);
                SessionDefault();
            }
            catch (Exception ex)
        ...
        protected void LoadDanhSach(int v_start, int v_end)
        {
            try
            {
                ...
                if (vSearchOption == "")
                {
                    vSearchOption = "|CHUCVU.TENCHUCVU,normal,,|CHUCVU.MOTA,normal,,";
                }
                DataSet ds = objCommonController.GetPage(PortalId, ModuleId, "ChucVu_GetPage", vSearchOption, textSearchContent.Text, vKeySort + " " + vTypeSort, v_start - 1, v_end);
