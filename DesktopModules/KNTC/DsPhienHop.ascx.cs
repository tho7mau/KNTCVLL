...
namespace HOPKHONGGIAY
{
    ...
    public partial class DsPhienHop : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Properties
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Get_Cache();
                //Edit Title
                this.ModuleConfiguration.ModuleTitle = "Quản lý phiên họp";
                if (!IsPostBack)
                {
                    try
                    {
        ...
        protected void dgDanhSach_Sua(object sender, EventArgs e)
        {
            ...
            //string vUrl = Globals.NavigateURL("edit", "mid=" + this.ModuleId, "title=Cập nhật thông tin phiên họp", "id=" + vId);
            string vUrl = Globals.NavigateURL("edit", "mid=" + this.ModuleId,  "id=" + vId);
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (vSearchContent != "" || vSearch_TrangThai != -1)
            {
                ...
                dictSearch.Add("DonVi", vSearch_TrangThai.ToString());               
            }
            if (vSearchContent == "")
            {
                if (dictSearch != null)
        ...
        public void SetFormInfo(Object pObjSearch)
        {
            if (pObjSearch != null)
            {
                ...
                {
                    ddlistTrangThai.SelectedValue = dictSearch["DonVi"];
                }               
            }
            else
            {
        ...
        protected void LoadDanhSach(int pCurentPage)
        {
            ...
            {                
                ...
                if (Session[vMacAddress + PortalSettings.ActiveTab.TabID + _currentUser.UserID + "_SearchContent"] != null)
                {
                    ...
                    {
                        vDONVI_ID = int.Parse(vDictSearch["DonVi"]);
                    }
                ...
                List<PHIENHOP> vPhienHopInfos = vPhienHopControllerInfo.GetDanhSachPhienHop_ChuaDuyet(vContentSearch, vDONVI_ID);
                ...
                if (ViewState["keysort_phienhop"] != null && ViewState["typesort_phienhop"] != null)
                {
                    ...
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderBy(x => x.EMAIL).ToList();
                    //if (key == "EMAIL" && type == "DESC")
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderByDescending(x => x.EMAIL).ToList();

                    //if (key == "SODIENTHOAI" && type == "ASC")
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderBy(x => x.SODIENTHOAI).ToList();
                    //if (key == "SODIENTHOAI" && type == "DESC")
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderByDescending(x => x.SODIENTHOAI).ToList();

                    if (key == "DONVI" && type == "ASC")
                        vPhienHopInfos = vPhienHopInfos.OrderBy(x => x.DONVI.TENDONVI).ToList();
                    if (key == "DONVI" && type == "DESC")
                        vPhienHopInfos = vPhienHopInfos.OrderByDescending(x => x.DONVI.TENDONVI).ToList();

                    //if (key == "CHUCVU" && type == "ASC")
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderBy(x => x.CHUCVU.TENCHUCVU).ToList();
                    //if (key == "CHUCVU" && type == "DESC")
                    //    vNguoiDungInfos = vNguoiDungInfos.OrderByDescending(x => x.CHUCVU.TENCHUCVU).ToList();
                }
        ...
        /// <summary>
        /// Thay đổi trạng thái sử dụng phiên họp
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void ThayDoiTrangThai(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (vPhienHopInfo.TRANGTHAI == 0 || vPhienHopInfo.TRANGTHAI == 6)
                    {
                    ...
                    ClassCommon.ShowToastr(Page, "Gửi duyệt phiên họp thành công!", "Thông báo", "Success");
                    LoadDanhSach(vCurentPage);
                }
        ...
        #region Show nội dung yêu cầu chỉnh sửa
        protected void showNoiDungYeuCauChinhSua(object sender, EventArgs e)
        {
                HtmlAnchor html = (HtmlAnchor)sender;
            ...
            if (objPHIENHOP !=null)
            {
                if (!string.IsNullOrEmpty(objPHIENHOP.GHICHUCANCHINHSUA))
                {
                    lblNoiDungCanChinhSua.Text = objPHIENHOP.GHICHUCANCHINHSUA;                  
                }
                else
                {
                    lblNoiDungCanChinhSua.Text = "Không có nội dung phản hồi";
                }
