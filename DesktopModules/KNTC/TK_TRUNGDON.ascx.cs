...
namespace KNTC
{
    public partial class TK_TRUNGDON : DotNetNuke.Entities.Modules.UserModuleBase
    {
        ...
        protected void btnXuatExel_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                try
                {
                    ...
                    {
                        Byte[] fileBytes = baoCaoController.TK_DONTHU_TRUNG(vTuNgay,vDenNgay, ddlistLoaiDoiTuong.SelectedValue);
                        if (fileBytes != null)
                        {
                            Response.Clear();
