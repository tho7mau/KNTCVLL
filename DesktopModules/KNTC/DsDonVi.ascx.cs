...
namespace KNTC
{
    ...
    public partial class DsDonVi : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Properties
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Get_Cache();
                //Edit Title
                this.ModuleConfiguration.ModuleTitle = "Quản lý đơn vị";
                if (!IsPostBack)
                {
        ...
        protected void dgDanhSach_Sua(object sender, EventArgs e)
        {
            ...
            string vUrl = Globals.NavigateURL("edit", "mid=" + this.ModuleId, "title=Cập nhật thông tin đơn vị", "id=" + vId);
        ...
        protected void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {                    
                    vDonViController.XoaDonVi(vId, out oErrorMessage);
                    vCountDaXoa += 1;
                }
                ClassCommon.ShowToastr(Page, "Xóa thành công " + vCountDaXoa + " đơn vị!", "Thông báo", "success");
        ...
        {
            string vUrl = Globals.NavigateURL("Edit", "mid=" + this.ModuleId, "title=Thêm mới đơn vị", "id=0");
            SessionDestroy(new string[] { vMacAddress + PortalSettings.ActiveTab.TabID + _currentUser.UserID + "_SearchContent" });
            Response.Redirect(vUrl);
        }
        ...
        protected void LoadDanhSach(int v_start, int v_end)
        {
            try
            {
                ...
                if (vSearchOption == "")
                {
                    vSearchOption = "|DONVI.TENDONVI,normal,,|DONVI.TENVIETTAT,normal,,|DONVI.MOTA,normal,,";
                }
                DataSet ds = objCommonController.GetPage(PortalId, ModuleId, "DonVi_GetPage", vSearchOption, textSearchContent.Text, vKeySort + " " + vTypeSort, v_start - 1, v_end);
