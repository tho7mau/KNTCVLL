...
namespace KNTC
{
    public partial class TK_KETQUATOCAO : DotNetNuke.Entities.Modules.UserModuleBase
    {
        ...
        protected void btnXuatExel_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                try
                {
                    ...
                    {
                        Byte[] fileBytes = baoCaoController.KQ_XULYDON_TOCAO(vTuNgay, vDenNgay, lblKyBaoCao.Text);
                        if (fileBytes != null)
                        {
                            Response.Clear();
                            Response.Buffer = true;
                            Response.AddHeader("content-disposition", "attachment;filename=" + "TK_BM03_XLD_" + DateTime.Now.Day + "_" + DateTime.Now.Month + "_"
