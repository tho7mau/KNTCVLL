...
/// Mục đích        :Hiển thị danh sách phòng họp
/// Ngày tại        :03/04/2020
...
{
    /// <summary>
    /// Xử lý dữ liệu danh sách phòng họp
    /// </summary>

    public partial class DsPhongHop : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Properties
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Get_Cache();
                //Edit Title
                this.ModuleConfiguration.ModuleTitle = "Quản lý phòng họp";
                if (!IsPostBack)
                {
                    try
                    {                      
        ...
        /// <summary>
        /// Cập nhật thông tin phòng họp (chuyển đến trang Edit)
        /// </summary>
        ...
        protected void dgDanhSach_Sua(object sender, EventArgs e)
        {
            ...
            string vUrl = Globals.NavigateURL("edit", "mid=" + this.ModuleId, "title=Cập nhật thông tin phòng họp", "id=" + vId);
        ...
        protected void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                //Lấy danh sách ID phòng họp đã chọn trên danh sách
                List<int> vPhongHopIds = new List<int>();
                foreach (DataGridItem GridItem in dgDanhSach.Items)
                {
                    ...
                    {
                        vPhongHopIds.Add((int.Parse(dgDanhSach.DataKeys[GridItem.ItemIndex].ToString())));
                    }
                }
                //Xóa danh sách phòng họp đã chọn
                int vCountPhongHopDaXoa = 0;
                string oErrorMessage = "";
                foreach (int vPhongHopId in vPhongHopIds)
                {
                    if (!vPhongHopControllerInfo.KiemTraPhongHopDaDuocSuDung(vPhongHopId, out oErrorMessage))
                    {
                        vPhongHopControllerInfo.XoaPhongHop(vPhongHopId, out oErrorMessage);
                        vCountPhongHopDaXoa += 1;
                    }
                }
                ClassCommon.ShowToastr(Page, "Xóa thành công " + vCountPhongHopDaXoa + " phòng họp!", "Thông báo", "success");
        ...
        {
            HtmlAnchor html = (HtmlAnchor)sender;
            int vPhongHopId = int.Parse(html.HRef);
            string oErrorMessage = "";
            UserInfo _User = new UserInfo();
            try
            {
                if (vPhongHopControllerInfo.KiemTraPhongHopDaDuocSuDung(vPhongHopId, out oErrorMessage))//Người dùng đã phát sinh dữ liệu
                {
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa không thành công, phòng họp đã được sử dụng!", "Thông báo", "error");
                }
                else
                {
                    vPhongHopControllerInfo.XoaPhongHop(vPhongHopId, out oErrorMessage);
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa phòng họp thành công!", "Thông báo", "success");
                    LoadDanhSach(vCurentPage);
                }
            }
            catch (Exception ex)
        ...
        protected void LoadDanhSach(int pCurentPage)
        {
            ...
            {                
                ...
                List<PHONGHOP> vPhongHopInfo = vPhongHopControllerInfo.GetDanhPhongHop(vContentSearch);

                if (ViewState["keysort_phonghop"] != null && ViewState["typesort_phonghop"] != null)
                {
                    string key = ViewState["keysort_phonghop"].ToString();
                    string type = ViewState["typesort_phonghop"].ToString();

                    if (key == "TenPhongHop" && type == "ASC")
                        vPhongHopInfo = vPhongHopInfo.OrderBy(x => x.TENPHONGHOP).ToList();
                    if (key == "TenPhongHop" && type == "DESC")
                        vPhongHopInfo = vPhongHopInfo.OrderByDescending(x => x.TENPHONGHOP).ToList();

                    if (key == "DienGiai" && type == "ASC")
                        vPhongHopInfo = vPhongHopInfo.OrderBy(x => x.DIENGIAI).ToList();
                    if (key == "DienGiai" && type == "DESC")
                        vPhongHopInfo = vPhongHopInfo.OrderByDescending(x => x.DIENGIAI).ToList();
                    

                    if (key == "SucChua" && type == "ASC")
                        vPhongHopInfo = vPhongHopInfo.OrderBy(x => x.SUCCHUA).ToList();
                    if (key == "SucChua" && type == "DESC")
                        vPhongHopInfo = vPhongHopInfo.OrderByDescending(x => x.SUCCHUA).ToList();
                }

                if (vPhongHopInfo != null)
                {
                    dgDanhSach.VirtualItemCount = vPhongHopInfo.Count;
                    dgDanhSach.DataSource = vPhongHopInfo;
        ...
        public string GetLichHopTiepTheo(int pPhongHopId)
        {
            return vPhongHopControllerInfo.GetLichHopTiepTheoTheo(pPhongHopId, 7);
        }
        ...
        protected void dgDanhSach_SortCommand(object source, DataGridSortCommandEventArgs e)
        {
            ...
            ViewState["keysort_phonghop"] = e.SortExpression;
            ViewState["typesort_phonghop"] = sortDirection;
            LoadDanhSach(0);
        }
