...
/// Mục đích        :Hiển thị danh sách chức vụ
/// Ngày tại        :03/04/2020
...
{
    /// <summary>
    /// Xử lý dữ liệu danh sách chức vụ
    /// </summary>

    public partial class DsChucVu : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Properties
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Get_Cache();
                //Edit Title
                this.ModuleConfiguration.ModuleTitle = "Quản lý chức vụ";
                if (!IsPostBack)
                {
                    try
                    {                      
        ...
        protected void dgDanhSach_Sua(object sender, EventArgs e)
        {
            ...
            string vUrl = Globals.NavigateURL("edit", "mid=" + this.ModuleId, "title=Cập nhật thông tin chức vụ", "id=" + vId);
        ...
        protected void btn_Xoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ClassCommon.ShowToastr(Page, "Xóa thành công " + vCountPhongHopDaXoa + " chức vụ!", "Thông báo", "success");
        ...
        protected void dgDanhSach_Xoa(object sender, EventArgs e)
        {
            ...
            try
            {
                if (vChucVuControllerInfo.KiemTraChucVuDangDuocSuDung(vChucVuId, out oErrorMessage))
                {
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa không thành công, chức vụ đã được sử dụng!", "Thông báo", "error");
                }
                else
                {
                    vChucVuControllerInfo.XoaChucVu(vChucVuId, out oErrorMessage);
                    ClassCommon.THONGBAO_TOASTR(Page, null, _User, "Xóa chức vụ thành công!", "Thông báo", "success");
                    LoadDanhSach(vCurentPage);
                }
            }
            catch (Exception ex)
        ...
        protected void LoadDanhSach(int pCurentPage)
        {
            ...
            {                
                ...
                List<CHUCVU> vChucVuInfos = vChucVuControllerInfo.GetDanhSachChucVu(vContentSearch);

                if (ViewState["keysort_chucvu"] != null && ViewState["typesort_chucvu"] != null)
                {
                    string key = ViewState["keysort_chucvu"].ToString();
                    string type = ViewState["typesort_chucvu"].ToString();

                    if (key == "TenChucVu" && type == "ASC")
                        vChucVuInfos = vChucVuInfos.OrderBy(x => x.TENCHUCVU).ToList();
                    if (key == "TenChucVu" && type == "DESC")
                        vChucVuInfos = vChucVuInfos.OrderByDescending(x => x.TENCHUCVU).ToList();

                    if (key == "MoTa" && type == "ASC")
                        vChucVuInfos = vChucVuInfos.OrderBy(x => x.MOTA).ToList();
                    if (key == "MoTa" && type == "DESC")
                        vChucVuInfos = vChucVuInfos.OrderByDescending(x => x.MOTA).ToList();                  

                }

                if (vChucVuInfos != null)
                {
                    dgDanhSach.VirtualItemCount = vChucVuInfos.Count;
                    dgDanhSach.DataSource = vChucVuInfos;
        ...
        protected void dgDanhSach_SortCommand(object source, DataGridSortCommandEventArgs e)
        {
            ...
            ViewState["keysort_chucvu"] = e.SortExpression;
            ViewState["typesort_chucvu"] = sortDirection;
            LoadDanhSach(0);
        }
